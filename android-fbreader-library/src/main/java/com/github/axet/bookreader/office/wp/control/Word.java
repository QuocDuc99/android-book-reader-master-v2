...
package com.github.axet.bookreader.office.wp.control;
...
import com.github.axet.bookreader.office.OnChangePageDocumentListener;
import com.github.axet.bookreader.office.common.IOfficeToPicture;
import com.github.axet.bookreader.office.common.picture.PictureKit;
...
import com.github.axet.bookreader.office.pg.animate.FadeAnimation;
import com.github.axet.bookreader.office.simpletext.control.Highlight;
import com.github.axet.bookreader.office.simpletext.control.IHighlight;
import com.github.axet.bookreader.office.simpletext.control.IWord;
...
import com.github.axet.bookreader.office.system.IDialogAction;
import com.github.axet.bookreader.office.system.SysKit;
import com.github.axet.bookreader.office.system.beans.pagelist.APageListView;
import com.github.axet.bookreader.office.wp.view.LayoutKit;
import com.github.axet.bookreader.office.wp.view.NormalRoot;
import com.github.axet.bookreader.office.wp.view.PageRoot;
import com.github.axet.bookreader.office.wp.view.PageView;
import com.github.axet.bookreader.office.wp.view.WPViewKit;
...
public class Word extends LinearLayout implements IWord
{
    ...
    private OnChangePageDocumentListener onChangePageDocumentListener;
     ...
     * @param context
     */
    public Word(Context context, IDocument doc, String filePath, IControl control, OnChangePageDocumentListener onChangePageDocumentListener)
    {
        super(context);
        this.heightScreen = Util.getScreenWidth(context);
        this.control = control;
        this.onChangePageDocumentListener = onChangePageDocumentListener;
        this.doc = doc;
    ...
    public int getCurrentPageNumber()
    {
        ...
        if(pageRoot.getPages().size()>0 && pageRoot.getPages().get(0).getHeight()>0){
            pageHeight = pageRoot.getPages().get(0).getHeight();
        }
        PageView pv = WPViewKit.instance().getPageView(pageRoot, (int)(getScrollX() / zoom),
            (int)(getScrollY() / zoom) + getHeight() / 3);
        float hieuso = 0.8f;
        if(zoom<=1) hieuso =1;
//        int heightScreen = Utils.getScreenHeight(getContext());
//        if(heightScreen > pageHeight*2*zoom) hieuso=heightScreen/(float)(pageHeight*zoom);
        if(pageHeight>0 && (getScrollY()+ heightScreen)>= pageHeight*(pageRoot.getPageCount() - hieuso)*zoom){
    ...
    {
        //Todo: Duongcv draw page doc, docx
...
//            canvas.drawText(pn, x, y, paint);
            int pageCurrent = getCurrentPageNumber();
            if(pageCurrent!=pageOld || pageCount != pageRoot.getPageCount()){
                pageOld = pageCurrent;
                pageCount = pageRoot.getPageCount();
                if(onChangePageDocumentListener!=null) onChangePageDocumentListener.changePage(pageCurrent, pageRoot.getPageCount());
            }
        }        
